...
export const generatePdfForTemplate19 = async (
  ...
  const drawStaticHeader = (): number => {
    ...
    if (invoiceData.company.state !== "N/A") {
      doc.text(`State: ${invoiceData.company.state}`, M, y);
    }
  ...
  // --------------------------------------------------------------------------
  // MODIFIED drawCustomerMetaBlock to match image 1 (Buyer/Consignee stacked on left, Meta on right)
  // --------------------------------------------------------------------------
  const drawCustomerMetaBlock = (startY: number): number => {
    let leftY = startY;
    const rightX = getW() - M - 120; // Anchor point for right column labels
    let rightY = startY;

    // =================================================================
    // RIGHT BLOCK: Invoice Meta (Invoice #, Date, PO, E-Way)
    // =================================================================
    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...DARK);
    ...
    for (let i = 0; i < metaLabels.length; i++) {
      doc.text(metaLabels[i], rightX - 30, rightY);
      doc.setFont("helvetica", "normal");
      doc.text(metaValues[i], rightX + 50, rightY);
      doc.setFont("helvetica", "bold");
      rightY += 14;
    }

    // LEFT BLOCK 1: Details of Buyer | Billed to (Party)
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(10);
    const HIGHLIGHT_COLOR: [number, number, number] = [118, 108, 128];
    const NORMAL_HEADER_COLOR: [number, number, number] = [0, 0, 0];
    doc.setTextColor(...HIGHLIGHT_COLOR); // <-- SET HIGHLIGHT COLOR
    doc.text("Details of Buyer | Billed to :", M, leftY);
    doc.setTextColor(...NORMAL_HEADER_COLOR); // <-- RESTORE NORMAL COLOR
    leftY += 16;

    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    doc.text(invoiceData.invoiceTo.name, M, leftY); // Bold name
    leftY += 12;

    doc.setFont("helvetica", "normal"); // Normal for address and details
    ...
    leftY += billAddressLines.length * 12 + 2; // +2 for spacing

    doc.setFontSize(8);
    if (invoiceData.company.phone !== "N/A") {
      doc.text(`Phone: ${invoiceData.company.phone}`, M, leftY);
      leftY += 12;
    }
    ...
    if (invoiceData.invoiceTo.state !== "N/A") {
      doc.text(`State: ${invoiceData.invoiceTo.state}`, M, leftY);
      leftY += 14;
    }

    
    // LEFT BLOCK 2: Details of Consignee | Shipped to (Shipping Address)
    ...
    doc.setFontSize(10);
    doc.setTextColor(...HIGHLIGHT_COLOR); // <-- SET HIGHLIGHT COLOR
    doc.text("Details of Consignee | Shipped to :", M, leftY);
    doc.setTextColor(...NORMAL_HEADER_COLOR); // <-- RESTORE NORMAL COLOR
    leftY += 16;

    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    doc.text(invoiceData.shippingAddress.name, M, leftY); // Bold name
    leftY += 12;
    ...
    if (shipAddressLines.length) doc.text(shipAddressLines, M, leftY);
    leftY += shipAddressLines.length * 12 + 2;

    doc.setFontSize(8);
    if (invoiceData.shippingAddress.state !== "N/A") {
      doc.text(`State: ${invoiceData.shippingAddress.state}`, M, leftY);
      leftY += 12;
    }

    // Return the maximum bottom Y position + margin
    return Math.max(leftY, rightY) + 10;
  };
  // --------------------------------------------------------------------------
  // END MODIFIED drawCustomerMetaBlock
  // --------------------------------------------------------------------------
  ...
  const bankDetails =
    bank && typeof bank === "object" && bank.bankName
      ? {
          name: bank.bankName,
          branch:
            (bank as any).branchName || bank.branchAddress || "Branch Name",
  ...
  putBankDetail("Bank Name:", bankDetails.name, bankDetailY);
  bankDetailY += 12;
  putBankDetail("Branch:", bankDetails.branch, bankDetailY);
  bankDetailY += 12;
  putBankDetail("Acc. Number:", bankDetails.accNumber, bankDetailY);
  bankDetailY += 12;
  putBankDetail("IFSC:", bankDetails.ifsc, bankDetailY);
  bankDetailY += 12;
  putBankDetail("UPI ID:", bankDetails.upiId, bankDetailY);
  bankDetailY += 12;
