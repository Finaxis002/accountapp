...
    const shipAddressLines = doc.splitTextToSize(addressString, 200); // Increased width since it's full column

    if (shipAddressLines.length) doc.text(shipAddressLines, M, shippingY);
    shippingY += shipAddressLines.length * 12; // ⭐ Apply capitalizeWords (State logic remains the same)

    if (invoiceData.shippingAddress.state !== "N/A") {
      doc.text(`State: ${invoiceData.shippingAddress.state}`, M, shippingY);
      shippingY += 14;
    }
    // --- MODIFICATION END ---
    
...
    return Math.max(contentBottomY, rightY) + 10;
  };
