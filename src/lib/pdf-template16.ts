...
import { capitalizeWords } from "./utils"; // Assuming this utility is imported
...
export const generatePdfForTemplate16 = async (
  ...
  const drawCustomerMetaBlock = (startY: number): number => {
    ...
    // Agar address mein "missing" ya "not available" jaisa kuch hai, ya fir woh khaali hai (assuming capitalizeWords returns 'Address Missing' or similar when data is empty/null)
    if (
      !addressToDisplay ||
      addressToDisplay.toLowerCase().includes("address missing") ||
      addressToDisplay.toLowerCase().includes("not available")
    ) {
      addressToDisplay = "-";
    }

    const shipAddressLines = doc.splitTextToSize(addressToDisplay, 180);
    // --- MODIFICATION END ---

    console.log("address", shipAddressLines);
