        ...
        <Card>
            ...
            <CardContent>
               {isLoading ? (
                        ...
                        <TableBody>
                            {users.map(user => (
                                <TableRow key={user._id}>
                                    ...
                                    <TableCell>
                                        <Badge variant="outline" className={cn(roleBadgeColors[user.role!], "capitalize")}>{user.role}</Badge>
                                    </TableCell>
                                    <TableCell>
                                        <div className="flex flex-wrap gap-1">
                                            {(user.companies && Array.isArray(user.companies)) ? (
                                                user.companies.map((companyId: any) => {
                                                    ...
                                                    return companyName ? (
                                                        <Badge key={id} variant="secondary">{companyName}</Badge>
                                                    ) : null;
                                                })
                                            ) : (
                                    ...
                                    <TableCell className="text-right flex">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent>
                                                <DropdownMenuItem onClick={() => handleOpenForm(user)}><Edit className="mr-2 h-4 w-4" /> Edit User</DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => handleOpenDeleteDialog(user)} className="text-destructive"><Trash2 className="mr-2 h-4 w-4" /> Delete User</DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </TableCell>
                                </TableRow>
                    ...
                    <p className="text-muted-foreground text-center py-8">No users found for this client.</p>
                )}
            </CardContent>
        </Card>
