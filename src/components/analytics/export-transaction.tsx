...
// lazy-load xlsx only when exporting (client-side)
const XLSXMod = await import("xlsx");
const XLSX = (XLSXMod as any).default ?? XLSXMod;  // <— important
...
export function ExportTransactions({ selectedClientId, companyMap, defaultCompanyId, onExported }: Props) {
  ...
  const normalizeRow = (t: any, type: TxType) => {
    const name =
      companyMap.get(t?.company?._id || t?.companyId || t?.company) || "";
    return {
      type,
      date: t.date || t.createdAt || "",
      company: name,
      party: t.partyName || t.customerName || t.vendorName || t.party || "",
      amount: typeof t.amount === "number" ? t.amount : (t.total || t.netAmount || ""),
      reference: t.invoiceNumber || t.billNumber || t.reference || t.voucherNo || "",
      description: t.description || t.narration || t.notes || "",
      _id: t._id || "",
    };
  };
...
//     return "";
//   };
//   const companyNameFromItem = (t: any) => companyMap.get(idOf(t.company)) || idOf(t.company) || "";

//   // exact headers you want
//   const HEADERS = [
//     "party",
//     "date",
//     "amount",
...
//   ] as const;
//   const safe = (v: any) => (v ?? "").toString().replace(/"/g, '""');
//   const toCSV = (rows: Record<(typeof HEADERS)[number], any>[]) =>
//     [HEADERS.join(","), ...rows.map(r => HEADERS.map(h => `"${safe(r[h])}"`).join(","))].join("\n");

//   // -------- type-aware mapping (fixes your blanks) --------
//   // ---- company id helper (keeps filter correct with populated docs) ----
// // const idOf = (v: any) => (typeof v === "string" ? v : v?.$oid || v?._id || v?.id || "");

// // ---- type-aware mapping (reads your Sales shape: totalAmount + items[]) ----
...
//       const d = typeof raw === "string" ? new Date(raw) : new Date(raw?.$date || raw);
...
//         ? t.items
//             .map((it: any) => it?.product?.name || it?.productName || idOf(it?.product))
//             .filter(Boolean)
//             .join(", ")
...
//     // If your API expects ?company= instead of ?companyId=, change here:
...
//     const rows: Record<(typeof HEADERS)[number], any>[] = [];

//     datasets.forEach((data, i) => {
//       const txType = urls[i].t;
//       const list = Array.isArray(data) ? data : (data?.entries ?? []);

//       // ensure company-wise even if server returns all
//       const filtered = byCompany
//         ? list.filter((it: any) => {
//             const cid = idOf(it.company);
//             return cid && cid === companyId;
//           })
//         : list;

//       filtered.forEach((it: any) => rows.push(normalizeByType(it, txType)));
//     });

//     if (!rows.length) throw new Error("No data to export for selected filters.");

//     const csv = toCSV(rows);
//     const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
//     const a = document.createElement("a");
//     const fnameCompany = byCompany ? (companyMap.get(companyId) || companyId) : "all-companies";
//     const filename = `transactions_${fnameCompany}_${new Date().toISOString().slice(0,10)}.csv`;
...
const handleExport = async () => {
  ...
  // ---- type-aware mapping (your latest working normalizer) ----
  const normalizeByType = (t: any, txType: TxType) => {
    const base: any = {
      party: t.party?.name || t.partyName || t.customerName || t.vendorName || idOf(t.party) || "",
  ...
  try {
    ...
    const byCompany = companyId !== "ALL";
    const qCompany = byCompany ? `?companyId=${companyId}` : ""; // change to ?company= if your API needs that
    const clientPath = `/by-client/${selectedClientId}`;
