...
const formSchema = z.object({
  ...
  selectedClient: z.string().min(1, "You must select a client."), // ✅ updated field
});
...
export function AdminCompanyForm({
  ...
  const getClientId = (client: Company["client"]) => {
    if (!client) return "";
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      selectedClient: company?.selectedClient?._id || company?.selectedClient || "",
    },
  });
