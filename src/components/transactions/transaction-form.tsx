...
export function TransactionForm({ onFormSubmit }: TransactionFormProps) {
  ...
  const fetchInitialData = React.useCallback(async () => {
      ...
      try {
          ...
          const [companiesRes, partiesRes, productsRes, vendorsRes] = await Promise.all([
            ...
            fetch("http://localhost:5000/api/vendors", { headers: { "Authorization": `Bearer ${token}` } })
          ]);

          if (!companiesRes.ok || !partiesRes.ok || !productsRes.ok || !vendorsRes.ok) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
        ...
        const payload = {
            ...values,
            vendor: values.party // For purchase, party field holds vendor ID
        };
  ...
  const handleCreateEntity = async (name: string) => {
    ...
    const endpoint = type === 'sales' ? 'api/parties' : 'api/vendors';
    const entityType = type === 'sales' ? 'Party' : 'Vendor';
    ...
    try {
        ...
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || `Failed to create ${entityType}.`);
        
        const newEntity = data.party || data.vendor;
        
        if (type === 'sales') {
            setParties(prev => [...prev, newEntity]);
        } else {
            setVendors(prev => [...prev, newEntity]);
        }
        
        form.setValue('party', newEntity._id, { shouldValidate: true });
        toast({ title: `${entityType} Created`, description: `${newEntity.name} has been added.` });
        return newEntity;
    } catch (error) {
        toast({ variant: 'destructive', title: `Failed to create ${entityType}`, description: error instanceof Error ? error.message : "Unknown error" });
        return null;
    }
  }
  ...
  const partyOptions = (Array.isArray(currentParties) ? currentParties : []).map(p => ({ value: p._id, label: p.name }));
                     ...
                     <FormField
                        ...
                        render={({ field }) => (
                            ...
                            <Combobox
                                ...
                                placeholder={`Select or create a ${type === 'sales' ? 'customer' : 'vendor'}...`}
                                searchPlaceholder={`Search ${type === 'sales' ? 'customers' : 'vendors'}...`}
                                noResultsText={`No ${type === 'sales' ? 'customer' : 'vendor'} found.`}
                                creatable
