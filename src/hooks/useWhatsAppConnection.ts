...
export function useWhatsAppConnection(): UseWhatsAppConnectionReturn {
  ...
  const checkConnection = useCallback(async (force = false) => {
    ...
    try {
      ...
      // Debug staff access issues
      if (connected === false && !canManage) {
        console.warn('⚠️ Staff user cannot access WhatsApp connection');
        const debugInfo = whatsappConnectionService.debugStaffAccess();
        console.log('🔍 Staff Access Debug:', debugInfo);
      }
